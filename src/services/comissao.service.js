...
async function recalcularComissoes() {
    ...
    async function ajustar(dealId, tipoAgente, nome, correto, pciLabel) {
        ...
        if (diff > 0) {
            await creditarCashback(nome, diff, `Ajuste comissão ${dealId} — ${pciLabel}`, dealId, tipoAgente);
        } else {
    ...
    for (const deal of vendidos) {
        ...
        try {
            ...
            if (revenda && revenda !== "?????") {
                await ajustar(dealId, "revenda", revenda, comissoes.revenda ? comissoes.revenda.valor : 0, pci);
            }
            if (comissoes.representante) {
                await ajustar(dealId, "representante", comissoes.representante.nome, comissoes.representante.valor, pci);
            }
            if (comissoes.vendedorInterno) {
                await ajustar(dealId, "vendedor_interno", comissoes.vendedorInterno.nome, comissoes.vendedorInterno.valor, pci);
            }
        } catch (err) {
            erros++;
...
module.exports = { recalcularComissoes };
